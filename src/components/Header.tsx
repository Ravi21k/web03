...
export const Header: React.FC<HeaderProps> = ({ 
  ...
  const userNavItems = [
    { id: 'home', label: 'Home', icon: Home },
    { id: 'dashboard', label: 'Service', icon: Wrench },
    { id: 'vehicles', label: 'Vehicles', icon: Truck },
    { id: 'materials', label: 'Materials', icon: Package },
    { id: 'contact', label: 'Contact Us', icon: MessageCircle },
            ...
            <nav className="hidden lg:flex space-x-8">
              {(user ? userNavItems : guestNavItems).map((item) => (
                <button
                  ...
                  className={`flex items-center space-x-2 text-gray-700 hover:text-yellow-600 transition-colors font-medium py-2 border-b-2 border-transparent hover:border-yellow-500 ${
            ...
            <div className="px-4 py-4 space-y-3">
              {(user ? [...userNavItems, { id: 'profile', label: 'Profile', icon: User }] : guestNavItems).map((item) => (
                <button
                  key={item.id}
                  onClick={() => onNavigate(item.id as ViewType)}
                  className={`flex items-center space-x-3 w-full text-left px-4 py-3 text-gray-700 hover:text-yellow-600 hover:bg-yellow-50 rounded-lg font-medium transition-colors ${
              ...
              {user && (
                <button
                  onClick={handleLogout}
                  className="flex items-center space-x-3 w-full text-left px-4 py-3 text-red-600 hover:bg-red-50 rounded-lg font-medium transition-colors"
                >
                  <LogOut size={16} />
                  <span>Logout</span>
                </button>
